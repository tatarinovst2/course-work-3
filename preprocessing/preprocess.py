...
def preprocess_text_file(input_filename: str | Path, output_filename: str | Path, is_multiprocessed: bool = False) -> None:
...
def divide_csv_file_into_chunks(input_filename: str | Path, chunks_amount: int = 7) -> None:
    print(f"Dividing {input_filename} into {chunks_amount} chunks...")
    with open(input_filename, 'r', encoding='utf-8') as input_file:
        reader = csv.DictReader(input_file, delimiter=',', quotechar='"')
        lines = []
        lines.append(reader.fieldnames)

        for row in reader:
            lines.append(x for x in row.values())

        chunk_size = len(lines) // chunks_amount
        for i in range(chunks_amount):
            with open(f'{input_filename}_{i}.csv', 'w', encoding='utf-8') as output_file:
                writer = csv.writer(output_file, delimiter=',', quotechar='"')
                writer.writerows(lines[i * chunk_size: (i + 1) * chunk_size])


if __name__ == "__main__":
    preprocess_text_file(ROOT_PATH / 'datasets' / 'test_2013.txt',
                         ROOT_PATH / 'datasets' / 'test_2013_preprocessed.txt',
                         False)
    preprocess_text_file(ROOT_PATH / 'datasets' / 'test_2014.txt',
                         ROOT_PATH / 'datasets' / 'test_2015_preprocessed.txt',
                         False)
    ...
    for year in range(2011, 2023):
        p = Process(target=preprocess_text_file,
                    args=(ROOT_PATH / 'datasets' / 'merged_dataset' / f'merged_{year}.csv',
                          ROOT_PATH / 'datasets' / 'merged_dataset' / f'merged_{year}_preprocessed_2.txt',
                          True))
        processes.append(p)
